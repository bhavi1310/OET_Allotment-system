...
$students_query = "SELECT * FROM course_allotment2";
...
foreach ($students as $student) {
    ...
    $year = $student['year'];
    $cgpa = $student['cgpa'];
    $preference1 = $student['course1'];
    $preference2 = $student['course2'];
    $preference3 = $student['course3'];

    $allocated_course = null;

    // Check if the student's first preference is available
    if ($available_seats[$preference1] > 0) {
        $allocated_course = $preference1;
        $available_seats[$preference1]--;
    } else {
        // Check if the student's second preference is available
        if ($available_seats[$preference2] > 0) {
            $allocated_course = $preference2;
            $available_seats[$preference2]--;
        } else {
            // Check if the student's third preference is available
            if ($available_seats[$preference3] > 0) {
                $allocated_course = $preference3;
                $available_seats[$preference3]--;
            } else {
	...
	// Insert the student's data into the database
        $insert_query = "INSERT INTO allocated_students2 (name, rollno, branch, year, cgpa, course) 
